...
x_train = torch.tensor([0.0, 1.0]).reshape(-1, 1)  # x_train = [[1], [1.5], [2], [3], [4], [5], [6]]
y_train = torch.tensor([1.0, 0.0]).reshape(-1, 1)  # y_train = [[5], [3.5], [3], [4], [3], [1.5], [2]]
...
table = plt.table(cellText=[[0, 1], [1, 0]],
                  colWidths=[0.1] * 3,
                  colLabels=["$x_1$", "$f(x)$"],
                  cellLoc="center",
                  loc="lower center")

table._cells[(1,1)]._text.set_text("${%.1f}$" % model.f(torch.Tensor([0])))
table._cells[(2,1)]._text.set_text("${%.1f}$" % model.f(torch.Tensor([1])))
